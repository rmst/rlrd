...
class MlpActionValue(Sequential):
	def __init__(self, observation_space, action_space, hidden_units, device):
		super().__init__(
			DelayedBranchedMlpModule(observation_space, action_space, device, is_Q_network=True), ReLU(),
...
class MlpPolicy(Sequential):
	def __init__(self, observation_space, action_space, hidden_units, device):
		super().__init__(
			DelayedBranchedMlpModule(observation_space, action_space, device, is_Q_network=False), ReLU(),
			Linear(hidden_units, hidden_units), ReLU(),
...
class Mlp(ActorModule):
	def __init__(self, observation_space, action_space, hidden_units: int = 256, num_critics: int = 2):
		super().__init__()
		assert isinstance(observation_space, gym.spaces.Tuple)
		self.critics = ModuleList(MlpActionValue(observation_space, action_space, hidden_units, self.device) for _ in range(num_critics))
		self.actor = MlpPolicy(observation_space, action_space, hidden_units, self.device)
